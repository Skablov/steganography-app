    ...
    try {
        if (fileObject.containerFile !== null && fileObject.messageFile !== null) {
            ...
            if (bmpBuffer.length - 78 <= secretImage.length) {
                ...
                alert(`
                Ошибка пользователя:
                Размер буфера секретного сообщения -
                -- превышает размер желаемого контейнера!
                `)
                console.log(`-------------End-------------`);
                return;
            ...
            const changeLeastSignificantBit = (bufferElement, bit) => {
                let tmp = '';
                flag == true ? tmp = bufferElement.substring(0, bufferElement.length - 7) + '111111' + bit : tmp = bufferElement.substring(0, bufferElement.length - 1) + bit;
            ...
            flag == true ? fs.writeFileSync('withNoise.bmp', bmpBuffer) : fs.writeFileSync('stegocont.bmp', bmpBuffer)
